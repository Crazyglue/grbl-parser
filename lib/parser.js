...
module.exports = class GrblParser extends EventDispatcher {
  ...
  parseData(string) {
    ...
    if (this.check.isStatusReport(data)) {
      ...
      this.dispatch(this.messageTypes.status, statusData)
    }
    else if (this.check.isSuccessResponse(data)) {
      var successData = this.statusExtractor.successReport(data)
      this.dispatch(this.messageTypes.success, successData)
    }
      ...
      this.dispatch(this.messageTypes.feedbackMessage, feedbackMessageData)
    }
